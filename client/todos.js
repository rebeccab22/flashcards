...
Session.setDefault('tag_menu', 'collapsed');
...
var toggleTags = function() {
  if(Session.equals('tag_menu', 'collapsed')){
    Session.set('tag_menu', 'expanded');
  }
  else {
    Session.set('tag_menu', 'collapsed');
  }
};
...
Template.todos.todos = function () {
  ...
  return Todos.find(sel, {sort: {text: 1}});
};
...
Template.todo_item.events({
  ...
  'dblclick .translate .translate-text': function (evt, tmpl) {
    Session.set('editing_itemname', this._id);
    Deps.flush(); // update DOM before focus
    activateInput(tmpl.find("#todo-translate-input"));
  },
...
Template.todo_item.events(okCancelEvents(
  ...
  {
    ok: function (value) {
      Todos.update(this._id, {$set: {translate: value}});
      Session.set('editing_itemname', null);
    },
    cancel: function () {
      Session.set('editing_itemname', null);
...
Template.tag_filter.collapsed = function () {
  return Session.equals('tag_menu', 'collapsed') ? 'collapsed' : '';
};
...
Template.tag_filter.events({
  ...
  'click .label-expanded': function() {
    console.log("TOGGLE E");
    toggleTags();
  },

  'click .label-collapsed': function() {
